        ...
        <div class="max-w-7xl mx-auto flex items-center gap-6 text-sm">
           ...
           {{ request()->routeIs('car-models.index') ? 'text-blue-900 border-b-2 border-blue-900' : 'text-gray-700 hover:text-blue-900' }}">
            ...
            <a href="{{ route('car-models.edit', $carModel) }}"
               class="py-3 font-medium
           {{ request()->routeIs('car-models.edit') ? 'text-blue-900 border-b-2 border-blue-900' : 'text-gray-700 hover:text-blue-900' }}">
                Edit Car Model
            </a>
...
@section('content')
    ...
    <div class="max-w-xl mx-auto bg-white border rounded shadow-sm">
        ...
        <div class="px-6 py-4 border-b">
            <h1 class="text-lg font-semibold">Edit Car Model</h1>
        </div>
        ...
        <div class="p-6">
            ...
            <form action="{{ route('car-models.update', $carModel) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <label class="block text-sm">
                    ...
                    <select name="brand_id" required
                        ...
                        @foreach($brands as $brand)
                            <option value="{{ $brand->id }}" @selected(old('brand_id', $carModel->brand_id) == $brand->id)>
                                {{ $brand->brand }}
                            </option>
                        @endforeach
                    </select>
