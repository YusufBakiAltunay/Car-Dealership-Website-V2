...
@section('content')
    ...
    <p><strong>Naam:</strong> {{$priceCar->price}}</p>
    <p><strong>Date :</strong>{{$priceCar->effects}}</p>

    <form action="{{ route('price-cars.destroy', $priceCar) }}" method="POST">
        @csrf
        @method('DELETE')
        ...
        <a href="{{ route('price-cars.index') }}">Annuleren</a>
    </form>
@endsection
