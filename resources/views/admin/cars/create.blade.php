        ...
        <div class="max-w-7xl mx-auto flex items-center gap-6 text-sm">
           ...
           {{ request()->routeIs('cars.index') ? 'text-blue-900 border-b-2 border-blue-900' : 'text-gray-700 hover:text-blue-900' }}">
            ...
            <a href="{{ route('cars.create') }}"
               class="py-3 font-medium
           {{ request()->routeIs('cars.create') ? 'text-blue-900 border-b-2 border-blue-900' : 'text-gray-700 hover:text-blue-900' }}">
                Add Car
            </a>
...
@section('content')
    ...
    <div class="max-w-xl mx-auto bg-white border rounded shadow-sm">
        ...
        <div class="px-6 py-4 border-b">
            <h1 class="text-lg font-semibold">Create Car</h1>
        </div>
            ...
            <form action="{{ route('cars.store') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="acceleration" value="{{ old('acceleration') }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="max_load" value="{{ old('max_load') }}"
                               class="mt-1 w-full border rounded px-3 py-2 text-sm">
                    </div>
                    ...
                    <div>
                        <label class="block text-sm text-gray-700">Starting Price (€)</label>
                        ...
                        <select name="car_model_id"
                            ...
                            @foreach($carModels as $carModel)
                                <option value="{{ $carModel->id }}"
                                        @selected(old('car_model_id') == $carModel->id)>
                                    {{ $carModel->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="fuel_type_id"
                            ...
                            @foreach($fuelTypes as $fuelType)
                                <option value="{{ $fuelType->id }}"
                                        @selected(old('fuel_type_id') == $fuelType->id)>
                                    {{ $fuelType->name }}
                                </option>
                            @endforeach
                        </select>
